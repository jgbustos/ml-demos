...
leftovers_re = regex.compile('|'.join(sorted(leftovers_dict.keys())), regex.IGNORECASE)
contractions_re = regex.compile('|'.join(sorted(contractions_re_keys)), regex.IGNORECASE)
unsafe_re = regex.compile(r"\b" + r"\b|\b".join(sorted(unsafe_dict)) + r"\b", regex.IGNORECASE)
...
slang_re = regex.compile(
    r"\b" + r"\b|\b".join(sorted(list(slang) + list(unsafe_dict))) + r"\b", regex.IGNORECASE
)

LIM_RE = regex.compile("['’]")
...
def fix(s, leftovers=True, slang=True):
    ...
    if not LIM_RE.search(s):
        if slang and slang_re.search(s):
            ...
            # ensure str like expected from regex.sub
            return str(s)
